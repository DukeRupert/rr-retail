...
<AuthContainer>
  {#if state === "signin"}
    <h3
      class="font-heading text-3xl text-gray-900 font-semibold text-center mb-4"
    >
    ...
    <form action="?/login" method="POST" use:loginEnhance>
      ...
      <label class="label">
        ...
        />
        {#if $loginErrors.email}<span class="invalid">{$loginErrors.email}</span
          >{/if}
      </label>
      <label class="label">
  ...
  {:else if state === "signup"}
    ...
    <form use:registerEnhance method="POST" action="?/register">
  ...
  {:else if state === "reset"}
    ...
    <form action="?/reset" method="POST" use:resetEnhance>
      ...
      <label class="label">
        ...
        />
        {#if $resetErrors.email}<span class="invalid">{$resetErrors.email}</span
          >{/if}
      </label>
      <label class="label">
        <div class="label-text">Password</div>
